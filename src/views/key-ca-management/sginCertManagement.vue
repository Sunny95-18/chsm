      ...
      <Card :bordered="false" dis-hover>
        ...
        <Modal v-model="isInputShow" title="证书导入" @on-ok="inputCert">
          <Form ref="inputCertForm" :model="inputCertForm" :label-width="100">
            <FormItem label="证书别名：" >
              <Input v-model="inputCertForm.certName" style="width: 200px" />
            </FormItem>
            <FormItem label="对应证书：">
              <Upload
                v-if="isShowUpload"
...
export default {
  ...
  methods: {
    ...
    inputCert() {
      const uploadData = {
        certName: this.inputCertForm.certName,
      ...
      this.$store.dispatch("upload", uploadData).then((res) => {
        ...
        if (resData && resData.code == "200") {
          this.$Message.success("签名证书上传成功!");
          this.loadData();
        } else {
          this.$Message.error("签名证书上传失败! "+resData.message);
        }
      });
    },
    batchImport() {
